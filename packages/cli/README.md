...
`devflow work start`, `devflow work ready`, `devflow work block`, and
...
`/handoffs.json` plus map views at `/maps` and `/maps.json`, with work item
detail routes at `/work/<id>` and `/work/<id>.json`.
