...
filtered by `--agent <name>`, `--work <id>`, `--since <iso-date>`, and limited
by positive-integer `--limit <n>`. Use `--json` for the stable agent contract or
omit it for a short terminal summary with active filters, attached session ids,
changed-file counts, limit totals, or manual note summaries. The text summary
also surfaces a compact warning count when local state has warnings. `devflow
