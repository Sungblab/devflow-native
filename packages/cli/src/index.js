...
async function renderDashboardServe(argsForCommand) {
  ...
  const server = createServer(async (request, response) => {
    ...
    response.writeHead(200, { "content-type": "text/html; charset=utf-8" });
    response.end(createDashboardServedHtml(summary), () => closeServerOnce(server, options.once));
  });
...
const DASHBOARD_WEB_CSS = `.devflow-dashboard-shell {
  min-height: 100vh;
}
...
const DASHBOARD_WEB_JS = `async function loadDashboard() {
...
function createDashboardServedHtml(summary) {
  return createDashboardHtml(summary)
...
function renderDashboardSessionsPage(summary) {
  const sessionRows = summary.sessions.recent
    .map(
      (session) => `<tr><td>${escapeHtml(session.agent)}</td><td>${escapeHtml(session.kind)}</td><td>${escapeHtml(session.workItemId ?? "none")}</td><td>${escapeHtml(session.summary ?? session.sessionId ?? "none")}</td></tr>`,
    )
    .join("");
...
<body>
  <main>
    ...
    <p>${escapeHtml(summary.sessions.counts.total)} total / ${escapeHtml(summary.sessions.counts.manualNotes)} manual / ${escapeHtml(summary.sessions.counts.attached)} attached</p>
    <table>
...
function renderDashboardWorkPage(workItem) {
  return `<!doctype html>
<html lang="en">
<head>
  <meta charset="utf-8">
  <meta name="viewport" content="width=device-width, initial-scale=1">
  <title>Devflow Work Detail</title>
  <style>
    body { margin: 0; font-family: Inter, ui-sans-serif, system-ui, -apple-system, BlinkMacSystemFont, "Segoe UI", sans-serif; background: #f6f7f9; color: #171a1f; }
    main { max-width: 880px; margin: 0 auto; padding: 32px 20px 48px; }
    h1 { margin: 0 0 8px; font-size: 32px; }
...
<body>
  <main>
    <h1>Devflow Work Detail</h1>
    <dl>
      <dt>ID</dt><dd>${escapeHtml(workItem.id)}</dd>
      <dt>Title</dt><dd>${escapeHtml(workItem.title)}</dd>
      <dt>Status</dt><dd>${escapeHtml(workItem.status)}</dd>
      <dt>Description</dt><dd>${escapeHtml(workItem.description ?? "none")}</dd>
      <dt>Blocked reason</dt><dd>${escapeHtml(workItem.blockedReason ?? "none")}</dd>
    </dl>
  </main>
</body>
</html>
