...
`devflow.work_block`, `devflow.work_unblock`, and `devflow.work_list` expose
the local work item registry to agent hosts. The write tools append
`work.created`, `work.started`, `work.ready`, `work.blocked`, and
`work.unblocked` events; the list tool derives current item status from local
