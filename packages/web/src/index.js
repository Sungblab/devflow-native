...
export function renderDashboardMapsPage(summary) {
  const maps = summary.maps ?? {};
  const mapRows = (maps.items ?? [])
    .map(
      (map) =>
        `<tr><td>${escapeHtml(map.title)}</td><td>${escapeHtml(map.id)}</td><td>${escapeHtml(map.path)}</td></tr>`,
    )
    .join("");
...
<body>
  <main>
    <h1>Devflow Maps</h1>
    <p>${escapeHtml(maps.counts?.total ?? 0)} architecture maps</p>
    <table>
...
export function renderDashboardWorkPage(workItem) {
  return `<!doctype html>
<html lang="en">
<head>
  <meta charset="utf-8">
